...
from mbed_os_tools.detect.main import create, mbed_os_support, mbed_lstools_os_info, mock_platform
...
def print_mbeds(mbeds, args, simple):
    ...
    if devices:
        ...
        print(pt.get_string(border=not simple, header=not simple, padding_width=1, sortby="platform_name_unique"))
...
def mbeds_as_json(mbeds, args):
    print(json.dumps(mbeds.list_mbeds(unique_names=True, read_details_txt=True), indent=4, sort_keys=True))


def json_by_target_id(mbeds, args):
    print(
        json.dumps(
            {m["target_id"]: m for m in mbeds.list_mbeds(unique_names=True, read_details_txt=True)},
            indent=4,
            sort_keys=True,
        )
    )
...
def parse_cli(to_parse):
    ...
    commands.add_argument(
        ...
        help="map attached targets from their target ID to their detailed information in JSON format",
    )
    commands.add_argument(
        "-p",
        "--json-platforms",
    ...
    commands.add_argument(
        ...
        help="map attached platform names to the number of attached boards in JSON format",
    )
    commands.add_argument(
        "-l",
        "--list",
        ...
        help="list all target IDs and their corresponding platform names understood by mbed-ls",
    )
    commands.add_argument(
        "--version", dest="command", action="store_const", const=print_version, help="print package version and exit"
    )
    commands.add_argument(
        "-m",
        "--mock",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="skip parsing and interpretation of the re-target file, `./mbedls.json`",
    )
    parser.add_argument(
        "-u",
        "--list-unmounted",
...
def start_logging():
    try:
        ...
        colorlog.basicConfig(format="%(log_color)s%(levelname)s%(reset)s:%(name)s:%(message)s")
    except ImportError:
        import logging
...
def mbedls_main():
    ...
    mbeds = create(
        skip_retarget=args.skip_retarget, list_unmounted=args.list_unmounted, force_mock=args.command is mock_platform
    )
