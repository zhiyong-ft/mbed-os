...
def init_logger():
    ...
    userlog.addHandler(logging.FileHandler(pathlib.Path.cwd() / "scancode_evaluate.log", mode="w"))
...
def license_check(scancode_output_path):
    ...
    for scancode_output_data_file in scancode_output_data["files"]:
        ...
        if not has_permissive_text_in_scancode_output(scancode_output_data_file["licenses"]):
            ...
            if scanned_file_content is None or has_exempted_spdx_identifier(scanned_file_content):
                continue
            else:
...
def parse_args():
    ...
    parser.add_argument("scancode_output_path", help="scancode-toolkit output json file")
...
def main():
    ...
    if pathlib.Path(args.scancode_output_path).is_file():
        sys.exit(ReturnCode.SUCCESS.value if license_check(args.scancode_output_path) == 0 else ReturnCode.ERROR.value)
    else:
