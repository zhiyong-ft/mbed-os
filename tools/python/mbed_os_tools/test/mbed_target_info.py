...
TARGET_INFO_MAPPING = {
    "default": {"program_cycle_s": 4, "binary_type": ".bin", "copy_method": "default", "reset_method": "default"},
    "K64F": {
        "yotta_targets": [
    ...
    "NUCLEO_F401RE": {
        "yotta_targets": [{"yotta_target": "st-nucleo-f401re-gcc", "mbed_toolchain": "GCC_ARM"}],
...
def get_mbed_targets_from_yotta_local_module(mbed_classic_name, yotta_targets_path="./yotta_targets"):
    ...
    gt_logger.gt_log(
        ...
        % (gt_logger.gt_bright(yotta_targets_path), gt_logger.gt_bright(mbed_classic_name.lower().strip()))
    )
            ...
            with open(path, "r") as data_file:
                ...
                if yotta_target_name:
                    ...
                    gt_logger.gt_log_tab(
                        ...
                        % (gt_logger.gt_bright(target_dir_name), gt_logger.gt_bright(yotta_target_name))
                    )
...
def parse_mbed_target_from_target_json(mbed_classic_name, target_json_data):
    if not target_json_data or "keywords" not in target_json_data or "name" not in target_json_data:
        return None
