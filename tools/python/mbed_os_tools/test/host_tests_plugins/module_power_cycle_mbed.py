...
class HostTestPluginPowerCycleResetMethod(HostTestPluginBase):
    ...
    def __get_mbed_tas_rm_addr(self):
        ...
        try:
            ...
            self.print_plugin_error(
                "HOST: Failed to read environment variable (" + str(e) + "). Can't perform hardware reset."
            )
    ...
    def __hw_reset(self, ip, port, target_id, device_info):
        ...
        switch_off_req = {
            "name": "switchResource",
            "sub_requests": [{"resource_type": "mbed_platform", "resource_id": target_id, "switch_command": "OFF"}],
        }

        switch_on_req = {
            "name": "switchResource",
            "sub_requests": [{"resource_type": "mbed_platform", "resource_id": target_id, "switch_command": "ON"}],
        }
        ...
        if "error" in switch_off_req["sub_requests"][0]:
            self.print_plugin_error(
                "HOST: Failed to reset target. error = %s" % switch_off_req["sub_requests"][0]["error"]
            )
            return result
        ...
        def poll_state(required_state):
            switch_state_req = {
                "name": "switchResource",
                "sub_requests": [
                    {"resource_type": "mbed_platform", "resource_id": target_id, "switch_command": "STATE"}
                ],
            }
            ...
            while (
                ...
                and (
                    ...
                    or (required_state == "ON" and resp["sub_requests"][0]["mount_point"] == "Not Connected")
                )
