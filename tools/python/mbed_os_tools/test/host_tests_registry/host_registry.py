...
class HostRegistry:
    ...
    def register_from_path(self, path, verbose=False):
        ...
        if path:
            ...
            if exists(path) and isdir(path):
                python_modules = [f for f in listdir(path) if isfile(join(path, f)) and f.endswith(".py")]
    ...
    def _add_module_to_registry(self, path, module_file, verbose):
        ...
        try:
            ...
            print("HOST: Error! While loading local host test module '%s'" % join(path, module_file))
            print("HOST: %s" % str(e))
            return
        if verbose:
        ...
        for name, obj in getmembers(mod):
            if isclass(obj) and issubclass(obj, BaseHostTest) and str(obj) != str(BaseHostTest):
                if obj.name:
                ...
                if verbose:
                    print("HOST: Found host test implementation: %s -|> %s" % (str(obj), str(BaseHostTest)))
