...
def phase_setup(ser):
    ...
    twopartprint("\t", "Got SVL Bootloader Version: " + str(int.from_bytes(packet["data"], "big")))
...
def phase_serial_port_help():
    ...
    print(args.port.upper() + " not found but we detected the following serial ports:")
    for dev in devices:
        if "CH340" in dev.description:
            print(dev.description + ": Likely an Arduino or derivative. Try " + dev.device + ".")
...
def main():
    try:
        ...
        verboseprint("Script version " + SCRIPT_VERSION_MAJOR + "." + SCRIPT_VERSION_MINOR)
...
# ******************************************************************************
def cli():
    parser = argparse.ArgumentParser(description="SparkFun Serial Bootloader for Artemis")

    parser.add_argument("port", help="Serial COMx Port")

    parser.add_argument("-b", dest="baud", default=115200, type=int, help="Baud Rate (default is 115200)")

    parser.add_argument("-f", dest="binfile", default="", help="Binary file to program into the target device")

    parser.add_argument("-v", "--verbose", default=0, help="Enable verbose output", action="store_true")

    parser.add_argument(
        "-t", "--timeout", default=0.50, help="Communication timeout in seconds (default 0.5)", type=float
    )
