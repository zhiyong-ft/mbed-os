...
SUPPORTED_TOOLCHAINS = list(TOOLCHAINS - set("uARM"))
SUPPORTED_IDES = [exp for exp in list(EXPORTERS) + list(EXPORTER_ALIASES) if exp != "cmsis" and exp != "zip"]
...
def target_cross_toolchain(allowed_targets, allowed_toolchains, features=[]):
    ...
    for target in allowed_targets:
        for toolchain in allowed_toolchains:
            if all(feature in TARGET_MAP[target].features for feature in features):
...
def target_cross_ide(allowed_targets, allowed_ides, features=[], toolchains=[]):
    ...
    for target in allowed_targets:
        for ide in allowed_ides:
            if (
                EXPORTERS[ide].is_target_supported(target)
                and (not toolchains or EXPORTERS[ide].TOOLCHAIN in toolchains)
                and all(feature in TARGET_MAP[target].features for feature in features)
            ):
                yield target, ide
    ...
    for example in config["examples"]:
        ...
        if example["export"]:
            for name in get_sub_examples_list(example):
                ...
                for target, ide in target_cross_ide(
        ...
        results[example["name"]] = [exported, pass_status, successes, export_failures, build_failures, build_skips]
                ...
                for target, toolchain in target_cross_toolchain(
                    ...
                    if cmake:
                        ...
                        build_command_seq = [
                            "mbed-cli compile -t {} -m {} -j {} {}".format(
                                toolchain, target, str(jobs), "-vv" if verbose else ""
                            )
                        ]
                        if profiles:
                    ...
                    if not failed_flag:
                        if example["test"]:
                            ...
                            if image:
                                image_info = [
                                    {"binary_type": "bootable", "path": normpath(join(name, image)), "compare_log": log}
                                ]
