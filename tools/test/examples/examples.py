...
def parse_args():
    ...
    parser.add_argument(
        "-e", "--example", help=("filter the examples used in the script"), type=argparse_many(lambda x: x), default=[]
    )
    ...
    (
        compile_cmd.add_argument(
            ...
            type=argparse_force_uppercase_type(SUPPORTED_TOOLCHAINS, "toolchain"),
        ),
    )
    compile_cmd.add_argument(
        "-m",
        "--mcu",
        help=("build for the given MCU (%s)" % ", ".join(official_target_names)),
        metavar="MCU",
        type=argparse_many(argparse_force_uppercase_type(official_target_names, "MCU")),
        default=official_target_names,
    )

    compile_cmd.add_argument("--profiles", nargs="+", metavar="profile", help="build profile(s)")
    ...
    compile_cmd.add_argument(
        "--cmake", action="store_true", dest="cmake", default=False, help="Use Cmake to build example"
    )
    compile_cmd.add_argument(
        "-v", "--verbose", action="store_true", dest="verbose", default=False, help="Verbose diagnostic output"
    )
...
def do_compile(args, config, examples):
    ...
    results = lib.compile_repos(
        config, args.toolchains, args.mcu, args.profiles, args.verbose, examples, args.cmake, args.jobs
    )
