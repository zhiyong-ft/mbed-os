...
class RSAPublic(KeyClass):
    ...
    def verify(self, signature, payload):
        ...
        return k.verify(
            signature=signature, data=payload, padding=PSS(mgf=MGF1(SHA256()), salt_length=32), algorithm=SHA256()
        )
...
class RSA(RSAPublic):
    ...
    def generate(key_size=2048):
        if key_size not in RSA_KEY_SIZES:
            raise RSAUsageError("Key size {} is not supported by MCUboot".format(key_size))
