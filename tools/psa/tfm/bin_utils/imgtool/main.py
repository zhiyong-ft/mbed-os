...
from .keys import RSAUsageError, ECDSAUsageError, Ed25519UsageError, X25519UsageError

MIN_PYTHON_VERSION = (3, 6)
if sys.version_info < MIN_PYTHON_VERSION:
    sys.exit("Python %s.%s or newer is required by imgtool." % MIN_PYTHON_VERSION)
...
@click.option("-l", "--lang", metavar="lang", default=valid_langs[0], type=click.Choice(valid_langs))
@click.option("-k", "--key", metavar="filename", required=True)
        ...
        if value.lower() == "auto":
            ...
            try:
                ...
                raise click.BadParameter(
                    ...
                    "prefixed with 0b/0B, 0o/0O, or 0x/0X as necessary.".format(value)
                )
...
def validate_header_size(ctx, param, value):
    ...
    if value < min_hdr_size:
        raise click.BadParameter("Minimum value for -H/--header-size is {}".format(min_hdr_size))
    return value
...
def get_dependencies(ctx, param, value):
    if value is not None:
        ...
        if len(images) == 0:
            raise click.BadParameter("Image dependency format is invalid: {}".format(value))
...
class BasedIntParamType(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        try:
            ...
            self.fail(
                "%s is not a valid integer. Please use code literals "
...
@click.option("-x", "--hex-addr", type=BasedIntParamType(), required=False, help="Adjust address in hex output file.")
@click.option(
    "-L",
    "--load-addr",
...
@click.option(
    "-F", "--rom-fixed", type=BasedIntParamType(), required=False, help="Set flash address the image is built for."
)
@click.option(
    "--save-enctlv",
    default=False,
...
@click.option(
    ...
    "--encrypt",
    metavar="filename",
    help="Encrypt image using the provided public key. (Not supported in direct-xip or ram-load mode.)",
)
@click.option(
    "--encrypt-keylen",
    default="128",
...
@click.option(
    "-e", "--endian", type=click.Choice(["little", "big"]), default="little", help="Select little or big endian"
)
@click.option("--overwrite-only", default=False, is_flag=True, help="Use overwrite-only instead of swap upgrades")
@click.option(
    "--boot-record",
    metavar="sw_type",
...
@click.option("-M", "--max-sectors", type=int, help="When padding allow for this amount of sectors (defaults to 128)")
@click.option(
    "--confirm", default=False, is_flag=True, help="When padding the image, mark it as confirmed (implies --pad)"
)
@click.option("--pad", default=False, is_flag=True, help="Pad image to --slot-size bytes, adding trailer magic")
@click.option(
    "-S",
    "--slot-size",
    type=BasedIntParamType(),
...
@click.option(
    "--pad-header", default=False, is_flag=True, help="Add --header-size zeroed bytes at the beginning of the image"
)
@click.option("-H", "--header-size", callback=validate_header_size, type=BasedIntParamType(), required=True)
@click.option(
    "--pad-sig", default=False, is_flag=True, help="Add 0-2 bytes of padding to ECDSA signature (for mcuboot <1.5)"
)
@click.option(
    "-d",
    "--dependencies",
...
@click.option("--align", type=click.Choice(["1", "2", "4", "8"]), required=True)
@click.option(
    "--public-key-format",
...
def sign(
    ...
    if enckey and key:
        if (isinstance(key, keys.ECDSA256P1) and not isinstance(enckey, keys.ECDSA256P1Public)) or (
            isinstance(key, keys.RSA) and not isinstance(enckey, keys.RSAPublic)
        ):
            # FIXME
    ...
    for tlv in custom_tlv:
        ...
        if tag in image.TLV_VALUES.values():
            raise click.UsageError("Custom TLV %s conflicts with predefined TLV." % hex(tag))
    ...
    img.create(key, public_key_format, enckey, dependencies, boot_record, custom_tlvs, int(encrypt_keylen))
