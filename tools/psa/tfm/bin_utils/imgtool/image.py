...
boot_magic = bytes([0x77, 0xC2, 0x95, 0xF3, 0x60, 0xD2, 0xEF, 0x7F, 0x35, 0x52, 0x50, 0x0F, 0x2C, 0xB6, 0x79, 0x80])
...
class Image:
    def __init__(
        ...
        if security_counter == "auto":
            ...
            self.security_counter = (self.version.major << 24) + (self.version.minor << 16) + self.version.revision
        else:
    ...
    def save(self, path, hex_addr=None):
        ...
        if ext == INTEL_HEX_EXT:
            ...
            if self.base_addr is None and hex_addr is None:
                raise click.UsageError("No address exists in input file neither was it provided by user")
            h = IntelHex()
            ...
            if self.pad:
                trailer_size = self._trailer_size(
                    self.align, self.max_sectors, self.overwrite_only, self.enckey, self.save_enctlv, self.enctlv_len
                )
                trailer_addr = (self.base_addr + self.slot_size) - trailer_size
                padding = bytearray([self.erased_val] * (trailer_size - len(boot_magic)))
    ...
    def check_header(self):
        if self.header_size > 0 and not self.pad_header:
            if any(v != 0 for v in self.payload[0 : self.header_size]):
                raise click.UsageError("Header padding was not requested and image does not start with zeros")
    ...
    def ecies_hkdf(self, enckey, plainkey):
        ...
        derived_key = HKDF(
            algorithm=hashes.SHA256(), length=48, salt=None, info=b"MCUBoot_ECIES_v1", backend=default_backend()
        ).derive(shared)
        encryptor = Cipher(
            algorithms.AES(derived_key[:16]), modes.CTR(bytes([0] * 16)), backend=default_backend()
        ).encryptor()
        cipherkey = encryptor.update(plainkey) + encryptor.finalize()
        mac = hmac.HMAC(derived_key[16:], hashes.SHA256(), backend=default_backend())
        mac.update(cipherkey)
        ciphermac = mac.finalize()
        if isinstance(enckey, ecdsa.ECDSA256P1Public):
            pubk = newpk.public_key().public_bytes(encoding=Encoding.X962, format=PublicFormat.UncompressedPoint)
        else:
            pubk = newpk.public_key().public_bytes(encoding=Encoding.Raw, format=PublicFormat.Raw)
        return cipherkey, ciphermac, pubk

    def create(
        self, key, public_key_format, enckey, dependencies=None, sw_type=None, custom_tlvs=None, encrypt_keylen=128
    ):
        self.enckey = enckey
        ...
        if sw_type is not None:
            if len(sw_type) > MAX_SW_TYPE_LENGTH:
                msg = (
                    "'{}' is too long ({} characters) for sw_type. Its maximum allowed length is 12 characters.".format(
                        sw_type, len(sw_type)
                    )
                )
                raise click.UsageError(msg)

            image_version = str(self.version.major) + "." + str(self.version.minor) + "." + str(self.version.revision)
            ...
            boot_record = create_sw_component_data(sw_type, image_version, "SHA256", digest, pubbytes)
        ...
        if enckey is not None:
            ...
            elif isinstance(enckey, (ecdsa.ECDSA256P1Public, x25519.X25519Public)):
    ...
    def _trailer_size(self, write_size, max_sectors, overwrite_only, enckey, save_enctlv, enctlv_len):
        ...
        if overwrite_only:
            ...
            if write_size not in set([1, 2, 4, 8]):
                raise click.BadParameter("Invalid alignment: {}".format(write_size))
